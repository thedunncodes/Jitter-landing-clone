...
export default function HamBurger(props: hamBurgerProps) {
  ...
  return (
    <div aria-label="hamburger-container" className={`${hide ? 'w-0 -translate-x-14' : 'w-14 -translate-x-0  animate-pulse-shift' } flex items-center justify-center transition-all duration-400 delay-200 ease-out`} >
        <button aria-label="hamburger" onClick={handleMenu} className={`${hide ? 'scale-0 animate-pulse-exit' : 'w-fit py-3 px-3 animate-pulse-in' } h-full ${ dark ? 'bg-prim' : `${toggle ? 'bg-prim' : 'bg-sec'}` } overflow-hidden transition-all duration-400 delay-200 hover:animate-pulse-grow flex item-center justify-center rounded-full cursor-pointer`} >
