...
export default function HorizontalScroll(props: HorizontalScrollProps) {
  ...
  const mobileX = useTransform(scrollYProgress, [0.5, 1], [`${(translateRef.current?.offsetWidth || 0) / 2}px`, `-${offsetSize}px`]);
  const mobItem1Scale = useTransform(scrollYProgress, [0, 0.5], [1, 0.8]); 
  ...
  return (
    <div className="w-full h-full p-3 overflow-hidden">
        <div className="sticky w-full top-0 bg-cwyan-500 h-full flex items-center">
            <motion.div ref={translateRef} className="w-full flex bg-yellow h-full lg:pl-0 gap-y-4 gap-x-11 min-[375px]:gap-x-15 md:gap-x-0" style={{ x: (vpWidth < 1024) ? mobileX : x, columnGap: (vpWidth < 1024) ? undefined : flexGapX }}>
                    <motion.div
                        aria-label="horizontal-scroll-item1"
                        style={(vpWidth >= 1024) ? { scaleX: itemScaleX, scaleY: itemScaleY } : {}}
                        className={`relative flex-shrink-0 w-[50%] border lg:w-full h-full flex items-center justify-center`}
                    >
                        <div className="h-full w-full flex items-center justify-center lg:block min-[360px]:max-w-[10rem] max-w-[8.5rem] sm:max-w-[14rem] lg:max-w-full" >
                    ...
                    <motion.div
                        ...
                        style={(vpWidth >= 1024) ? { scaleX: itemScaleX, scaleY: itemScaleY } : {}}
                        className={`flex-shrink-0 w-[50%] border lg:w-full lg:min-w-[16rem] h-full flex items-scenter justify-center`}
                    >
                        <div className="w-full h-full flex border flex-wrap items-center justify-center" >
