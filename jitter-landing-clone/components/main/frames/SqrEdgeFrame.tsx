...
export default function SqrEdgeFrame(props: SqrEdgeFrameProps) {
    const { title, sqrEdge, children, scrollYProgress, scrollAnimation } = props;
    const full = useMotionValue(1);
    const scale = useTransform(scrollYProgress?? full, [0, 0.5], [0.3, 0.2]);

    return (
        <div aria-label="sqr-edge-container" className="w-fit h-fit relative" >
            {
                title && (
            ...
            }
            {
                sqrEdge && (
                    <>
                        ...
                        <motion.div style={ scrollAnimation? {scaleY: 0.06, y: '-50%'} : {}} aria-label="sqr-border" className="absolute z-1 w-full h-1 top-0 bg-prim-alt-dark" />
                        <motion.div style={ scrollAnimation? {scaleX: 0.06, x: '50%'} : {}} aria-label="sqr-border" className="absolute z-1 w-1 h-full -right-0 bg-prim-alt-dark" />
                        <motion.div style={ scrollAnimation? {scaleY: 0.06, y: '40%'} : {}} aria-label="sqr-border" className="absolute z-1 w-full h-1 bottom-0 bg-prim-alt-dark" />
                        <motion.div style={ scrollAnimation? {scaleX: 0.06, x: '-50%'} : {}} aria-label="sqr-border" className="absolute z-1 w-1 h-full -left-0 bg-prim-alt-dark" />
                    </>
                )
            }
